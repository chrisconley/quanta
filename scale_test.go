...
func TestNewScaleFrom(t *testing.T) {
	...
	t.Run("constructs from already-valid domain objects", func(t *testing.T) {
		s, err := NewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		require.NoError(t, err)
...
func TestMustNewScaleFrom(t *testing.T) {
	tokens := MustNewUnit("tokens", "1")
	usd := MustNewUnit("USD", "0.01")

	t.Run("returns scale on valid inputs", func(t *testing.T) {
		s := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		assert.True(t, s.InputUnit().Equal(tokens))
		assert.True(t, s.OutputUnit().Equal(usd))
		assert.True(t, s.Factor().Equal(MustNewDecimal("0.05")))
	})
...
func TestScale_Apply(t *testing.T) {
	...
	t.Run("preserves precision (no rounding)", func(t *testing.T) {
		s := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.033333"))
	...
	t.Run("zero factor yields zero result", func(t *testing.T) {
		s := MustNewScaleFrom(tokens, usd, Zero())
		input := NewMeasureFrom(tokens, MustNewDecimal("100"))
		out, err := s.Apply(input)
		require.NoError(t, err)
...
func TestScale_Equality(t *testing.T) {
	...
	t.Run("equal scales", func(t *testing.T) {
		a := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		b := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		assert.True(t, a.Equal(b))
	})

	t.Run("different factor", func(t *testing.T) {
		a := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		b := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.10"))
	...
	t.Run("different input unit", func(t *testing.T) {
		credits := MustNewUnit("credits", "1")
		a := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		b := MustNewScaleFrom(credits, usd, MustNewDecimal("0.05"))
		assert.False(t, a.Equal(b))
	})
	...
	t.Run("different output unit", func(t *testing.T) {
		...
		a := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		b := MustNewScaleFrom(tokens, eur, MustNewDecimal("0.05"))
		assert.False(t, a.Equal(b))
	})
	...
	t.Run("different quantum on same code not equal", func(t *testing.T) {
		...
		a := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		b := MustNewScaleFrom(tokens, usdFine, MustNewDecimal("0.05"))
		assert.False(t, a.Equal(b))
	})
}
...
func TestScale_String(t *testing.T) {
	...
	t.Run("renders input, output, and factor", func(t *testing.T) {
		s := MustNewScaleFrom(tokens, usd, MustNewDecimal("0.05"))
		assert.Equal(t, "Scale[tokens[q=1] -> USD[q=0.01] @ 0.05]", s.String())
	})
}
